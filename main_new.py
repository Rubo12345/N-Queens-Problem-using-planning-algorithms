...
def Astar(init_board_state, board_size):
    ...
    Open_List.append(Node(init_board_state))
    ...
    while Open_List:
        ...
        m = Open_List.pop(0)
        current_state = m[0]
        f_cost = m[1] #total cost ravelled so far
        g_cost = m[2] #cost+move+heuristic
        ...
        for i in range(len(queens.positions)):
            for queen in queens.positions:
                for j in range(-board_size+1, board_size):
                    ...
                    is_in_Open_List = any(np.array_equal(new_state,x) for x in Open_List)
